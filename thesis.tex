...
\begin{itemize}
\item Used in machine translation
\item Dual RNN architechture where 1st RNN encodes a sequence of tokens to fixed length vector and 2nd RNN decodeds that vector representation to a target sequence of tokens. \cite{Cho2014}
\item Both RNNs are jointly trained to maximize conditional probability of a target sequence given a source sequence. \cite{Cho2014}
\item Encoder creates a summary of the entire input sequence. \cite{Cho2014}
\item Decoder samples a token at a time using input sequence summary, it's own RNN hidden state and previous generated sample. \cite{Cho2014}
...
\item We selected \verb£ !"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz{|}~¤§®´ÄÅÖáãäåéíóöüš̈д–—’“”…€£
\item This contains ASCII from 32 to 127, scandic character in lower and upper case and 83 (99.99\% coverage) most frequent characters in Finnish internet parsebank
\item Selected character vocabulary covers 99,9919\% of character usages in Finnish internet parsebank n-gram dataset. N-gram data does not contain uppercased letters, those are in the ASCII set.
\end{itemize}
