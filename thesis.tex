...
Hypothesis of this thesis is that doing joint learning for lemmas and POS-tags will improve the classification performance of either or both.

\section{Reasoning for Choises}
\subsection{Lemmatization and POS-tagging}
\begin{itemize}
\item Lemmatization not solved for morphologically rich languages such as Finnish
\item Finnish was selected for it's difficulty and a practical reason that its author's native language
\item Very beneficial as input features for several downstream processing tasks
\end{itemize}

\subsection{Neural Networks}
\begin{itemize}
\item Old methods have reached their performance cap
\item Lots of promising results with neural networks during last couple of years
\item Don't require architectural tuning for each language (except hyperparameters)
\item Easy to implement additional tasks or features
\item RNNs are natural architecture for time series (text)
\item Character level models are tricky to implement with other methods
\end{itemize}

\subsection{Why Joint model?}
\begin{itemize}
...
\item Computational complexity decreases significantly when encoding only once for both with two linear output layers. Cost is the softmax. If POS-tagging is the auxiliary task then softmax is cheap.
\item Joint model is architecturally simple because both tasks share majority of the neural network. Also simple to implement.
\item Two flies and so forth...
\item If joint model is better then it's worth using even for a single task.
\end{itemize}

\section{Neural Network Architechture}
\label{se:architecture}
\begin{itemize}
\item Implemented in Python, Tensorflow and Numpy
\item Tensorflow makes it easy to implement complex neural networks as computational graphs without having to worry about implementing the back-propagation
\item Tensorflow allows to use NVIDIA GPUs, CPU-only would not have been feasible with the resources available for this thesis. Or at leas results would have been significantly worse.
